...
public class TurtleService implements Runnable {
	...
	public void run() {
		while (state != 2) {	
			...
			// state == 1일 때, 떡 state == 3, 사라짐, 점수 올라감
			checkCollision();
			
			if (turtle.getState() == 0) checkAttacked();
		...
		private void checkAttacked() {
        	// 공격 중임을 300ms 동안 유지 (300ms 동안 공격 이미지 유지하므로)
        	...
        	if (isAttacking && !isColliding && turtle.getState() == 0) {
        	    ...
        	    if (player.isLeft()) { // 왼쪽으로 공격할 때
        	        isAttacked = (playerX - 100 < turtleX && turtleX < playerX) && 
        	                     (turtleY < playerY + 40 && turtleY + 50 > playerY); // 왼쪽 공격 범위
        	    } else if (player.isRight()) { // 오른쪽으로 공격할 때
        	        isAttacked = (playerX + 50 < turtleX && turtleX < playerX + 150) && 
        	                     (turtleY < playerY + 40 && turtleY + 80 > playerY); // 오른쪽 공격 범위
        	    }
        	    
                // 디버깅용 출력 (공격 범위와 충돌 체크)
        	    if (player.isLeft()) System.out.println("공격 범위 체크: " + (playerX - 50) + " ~ " + playerX);
        	    else System.out.println("공격 범위 체크: " + (playerX + 50) + " ~ " + (playerX + 150));
